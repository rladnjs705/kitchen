...
/*			여기서 1페이지는 1SP(smallPage)		1~10페이지를 묶어서 1LP(largePage)						*/
public class MyUtil {
    //********************************************
	// 총페이지 수 구하기
	public int pageCount(int numPerPage, int dataCount) {//numPerPage: 표시할데이터 갯수
														 //dataCount: 총데이터
		
		int pageCount=0;//리턴 해 주는 값 초기화
		// 여기서 말하는 총페이지란 1~10 이  1LP라고 할때 의 경우다.
		
		
		if(dataCount > 0) {//데이터가 하나라도 있으면
			if(dataCount % numPerPage == 0)//데이터를 표시할 데이터 만큼 묶어서 
										   //페이지로 만들면 몇개나오는지 계산
										   //이유: 총데이터를 numPerPage로 그냥 나눠 버리면 나머지가 표시되지않음
				pageCount=dataCount/numPerPage;//딱 나누어 떨어지면..
			else
	...
	public String paging(int current_page, int total_page, String list_url) {
		//current_page: 현재 표시하는 페이지 숫자 SP
		//total_page: 전체페이지 SP
		//list_url:  예) /WEB-INF/views/bbs/list.do;
		StringBuffer sb=new StringBuffer();
		
		int numPerBlock=10;//SP 의 갯수
		int currentPageSetup;//????????
		int n, page;//???????
		
		if(current_page<1 || total_page < 1)//있을 수 없는 페이지를 확인한다...?
			return "";
		
		if(list_url.indexOf("?")!=-1)//뒤에 조건이 있나 확인
			list_url+="&";// 있으면 &(그리고)를 붙여준다. 
						  //예)/WEB-INF/views/bbs/list.do?searchKey=subject&searchValue='아아'
						  //결과 예)/WEB-INF/views/bbs/list.do?searchKey=subject&searchValue='아아'&
		else
			list_url+="?";//없으면 ?
						  //예) /WEB-INF/views/bbs/list.do
						  //결과 예)/WEB-INF/views/bbs/list.do?
